...
export const useOfflineQueue = (options: UseOfflineQueueOptions) => {
  ...
  const addToQueue = useCallback(async (operation: OfflineOperation): Promise<string> => {
    try {
  ...
  const getQueueItems = useCallback(async (filters?: any): Promise<QueueItem[]> => {
  ...
  const resolveConflict = useCallback(async (
    ...
    await offlineQueueManager.resolveConflict(itemId, resolution, mergedData, userId);
  }, [userId]);
  ...
  const createTransaction = useCallback(() => {
    ...
    return {
      ...
      commit: async (priority: 'LOW' | 'NORMAL' | 'HIGH' | 'CRITICAL' = 'NORMAL') => {
  ...
  const createOfflineWrapper = useCallback((collection: string) => ({
    ...
    delete: (documentId: string, priority?: 'LOW' | 'NORMAL' | 'HIGH' | 'CRITICAL') =>
      deleteDocument(collection, documentId, priority),
    
    batch: (operations: any[], priority?: 'LOW' | 'NORMAL' | 'HIGH' | 'CRITICAL') =>
      batchOperation(operations.map(op => ({ ...op, collection })), priority)
