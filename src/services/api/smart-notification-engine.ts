...
export interface SmartNotification {
  id: string;
  type: 'weather' | 'health' | 'video' | 'practice' | 'system' | 'team' | 'player';
  title: string;
  message: string;
  priority: NotificationPriority;
  category: string;
  recipientId: string;
  recipientType: 'head-coach' | 'assistant-coach' | 'player' | 'parent' | 'admin';
...
class SmartNotificationEngine {
    ...
    for (const member of teamMembers) {
      ...
      if (priority.score >= this.getNotificationThreshold(member.role)) {
        const notification = this.createNotification({
          type: 'weather',
          title: this.generateWeatherNotificationTitle(weatherData, recommendation),
          message: this.generateWeatherNotificationMessage(weatherData, recommendation),
          priority,
  ...
  async createHealthMonitoringNotification(
    ...
    for (const recipient of recipients) {
      ...
      if (priority.score >= this.getNotificationThreshold(recipient.role)) {
        const notification = this.createNotification({
          type: 'health',
          title: this.generateHealthNotificationTitle(healthData, riskAssessment),
          message: this.generateHealthNotificationMessage(healthData, riskAssessment),
          priority,
  ...
  private calculateWeatherNotificationPriority(
    ...
    // Time-based urgency
    const hoursUntilPractice = (practiceDate.getTime() - Date.now()) / (1000 * 60 * 60);
  ...
  private determineChannels(
    ...
    if (priority.level === 'critical') {
      if (preferences.channels.push) channels.push({ type: 'push', config: {}, isEnabled: true, status: 'pending' });
      if (preferences.channels.email) channels.push({ type: 'email', config: {}, isEnabled: true, status: 'pending' });
      if (preferences.channels.sms) channels.push({ type: 'sms', config: {}, isEnabled: true, status: 'pending' });
      if (preferences.channels.inApp) channels.push({ type: 'in-app', config: {}, isEnabled: true, status: 'pending' });
      return channels;
    }
  ...
  private createNotification(data: Partial<SmartNotification>): SmartNotification {
  ...
  private async sendWebhookNotification(notification: SmartNotification): Promise<void> {
  ...
  // **Utility Methods**
  private generateWeatherNotificationTitle(weatherData: any, recommendation: any): string {
    if (recommendation.riskLevel === 'high') {
  ...
  private generateWeatherNotificationMessage(weatherData: any, recommendation: any): string {
    if (recommendation.riskLevel === 'high') {
  ...
  private generateHealthNotificationTitle(healthData: any, riskAssessment: any): string {
    if (riskAssessment.riskLevel === 'high') {
  ...
  private generateHealthNotificationMessage(healthData: any, riskAssessment: any): string {
    if (riskAssessment.riskLevel === 'high') {
  ...
  private shouldReceiveVideoNotification(member: any, analysis: any): boolean {
    ...
    if (member.role === 'player' && analysis.playerId === member.id) return true;
  ...
  private getUserPreferences(role: string): NotificationPreferences {
    ...
    return {
      ...
      userType: role,
      categories: {
        weather: { isEnabled: true, priority: 'medium', channels: ['push', 'in-app'], quietHours: false, frequency: 'immediate' },
        health: { isEnabled: true, priority: 'high', channels: ['push', 'email', 'in-app'], quietHours: false, frequency: 'immediate' },
        video: { isEnabled: true, priority: 'medium', channels: ['in-app'], quietHours: true, frequency: 'batched' },
        practice: { isEnabled: true, priority: 'high', channels: ['push', 'email', 'in-app'], quietHours: false, frequency: 'immediate' },
        system: { isEnabled: true, priority: 'low', channels: ['in-app'], quietHours: true, frequency: 'daily' },
        team: { isEnabled: true, priority: 'medium', channels: ['push', 'in-app'], quietHours: true, frequency: 'batched' },
        player: { isEnabled: true, priority: 'medium', channels: ['in-app'], quietHours: true, frequency: 'batched' },
      },
      channels: {
        push: true,
        email: true,
  ...
  private scheduleNotification(notification: SmartNotification): void {
    ...
    scheduledTime.setMinutes(parseInt(preferences.quietHours.end.split(':')[1]));
  ...
  private async getHealthNotificationRecipients(playerId: string): Promise<any[]> {
  ...
  private initializeDefaultPreferences(): void {
    ...
    const userTypes = ['head-coach', 'assistant-coach', 'player', 'parent', 'admin'];
  ...
  async getNotificationStatus(notificationId: string): Promise<SmartNotification | undefined> {
  ...
  async updateNotificationPreferences(
    ...
    const current = this.preferences.get(userType) || this.getUserPreferences(userType);
  ...
  async getNotificationStats(): Promise<{
    ...
    return {
      ...
      critical: notifications.filter(n => n.priority.level === 'critical').length,
