...
exports.generateEnvExample = exports.getEnvironmentDocs = exports.validateVariable = exports.validateEnvironment = exports.environmentValidator = exports.EnvironmentValidator = void 0;
/**
...
class EnvironmentValidator {
    ...
    validate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const check of this.securityChecks) {
                try {
                    const isValid = yield check.check();
                    if (!isValid) {
                        errors.push({
                            variable: check.name,
    ...
    generateSuggestions(errors, warnings) {
        ...
        if (firebaseErrors.length > 0) {
            suggestions.push('📱 Firebase Setup: Visit https://console.firebase.google.com to create a project and get your configuration', '📋 Copy the Firebase config from Project Settings > General > Your Apps');
        }
        // OpenAI-related suggestions
        const openaiErrors = errors.filter(e => e.variable.includes('OPENAI'));
        if (openaiErrors.length > 0) {
            suggestions.push('🤖 OpenAI Setup: Visit https://platform.openai.com/api-keys to create an API key', '🔐 Keep your API key secure and never commit it to version control');
        }
        // Development suggestions
        if (process.env.NODE_ENV === 'development') {
            suggestions.push('🔧 Development Mode: Consider using Firebase emulators for local development', '📝 Create a .env.local file for local environment variables');
        }
        // Production suggestions
        if (process.env.NODE_ENV === 'production') {
            suggestions.push('🚀 Production Mode: Ensure all security features are enabled', '📊 Set up monitoring and analytics for production deployment', '🔒 Review and update security rules before deployment');
        }
        // General suggestions
        if (warnings.length > 0) {
            suggestions.push('⚙️ Optional Features: Consider enabling optional features for enhanced functionality', '📈 Analytics: Set up Sentry for error tracking in production');
        }
        return suggestions;
    }
    /**
...
exports.environmentValidator = new EnvironmentValidator();
// Export convenience functions
const validateEnvironment = () => exports.environmentValidator.validate();
exports.validateEnvironment = validateEnvironment;
const validateVariable = (variable, value) => exports.environmentValidator.validateVariable(variable, value);
exports.validateVariable = validateVariable;
const getEnvironmentDocs = () => exports.environmentValidator.getDocumentation();
exports.getEnvironmentDocs = getEnvironmentDocs;
const generateEnvExample = () => exports.environmentValidator.generateEnvExample();
