...
export const RouteDrawer: React.FC<RouteDrawerProps> = ({
  ...
  const [routeType, setRouteType] = useState<'run' | 'pass' | 'block' | 'custom'>('run');
  ...
  const continueDrawing = useCallback((e: any) => {
    ...
    const pos = stage.getPointerPosition();
    const newPoint: RoutePoint = { x: pos.x, y: pos.y, timestamp: Date.now() };
  ...
  const getLinePoints = useCallback((points: RoutePoint[]) => {
    return points.reduce((acc: number[], point) => [...acc, point.x, point.y], []);
  }, []);
  ...
  const renderRouteTypeSelector = () => {
    ...
    return (
      <div style={{
        ...
        <div style={{ marginBottom: '8px', fontWeight: 'bold' }}>Route Type:</div>
        {Object.keys(routeConfigs).map((type) => (
          <button
            ...
            style={{
              ...
              borderRadius: '4px',
              background: routeType === type ? routeConfigs[type as keyof typeof routeConfigs].color : 'white',
