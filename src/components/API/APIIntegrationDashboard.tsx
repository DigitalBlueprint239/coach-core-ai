...
const APIIntegrationDashboard: React.FC = () => {
  ...
  const getIntegrationIcon = (name: string) => {
    switch (name) {
      ...
      case 'hudl': return Video;
      case 'youtube': return Video;
      case 'vimeo': return Video;
      case 'fitbit': return Heart;
      case 'garmin': return Watch;
      case 'apple-healthkit': return Smartphone;
              ...
              <HStack spacing={4}>
                <Stat>
                  ...
                  <StatNumber color="green.600">
                    {statuses.filter(s => s.isEnabled && s.status === 'healthy').length}
                  </StatNumber>
                </Stat>
                <Stat>
                  <StatLabel color="gray.600">Warning</StatLabel>
                  <StatNumber color="yellow.600">
                    {statuses.filter(s => s.isEnabled && s.status === 'warning').length}
                  </StatNumber>
                </Stat>
                <Stat>
                  <StatLabel color="gray.600">Error</StatLabel>
                  <StatNumber color="red.600">
                    {statuses.filter(s => s.isEnabled && s.status === 'error').length}
                  </StatNumber>
                </Stat>
              </HStack>
            </HStack>
        ...
        <TabPanels>
            ...
            <Grid templateColumns={{ base: '1fr', lg: '2fr 1fr' }} gap={8}>
              ...
              <VStack spacing={6} align="stretch">
                ...
                {integrations.map((integration) => {
                  const status = statuses.find(s => s.name === integration.name);
                  return (
                    <Card key={integration.name} bg={bgColor} border="1px" borderColor={borderColor} shadow="sm">
                      <CardBody>
                        <HStack spacing={4} align="center">
                          ...
                          <Box flex={1}>
                            <HStack justify="space-between" mb={2}>
                              ...
                              <Badge 
                                colorScheme={getStatusColor(status?.status || 'disconnected')}
                                variant="subtle"
                              >
                            ...
                            <HStack spacing={4} fontSize="xs" color="gray.500">
                              <Text>Auto-sync: {integration.autoSync ? 'Yes' : 'No'}</Text>
                              {integration.autoSync && (
                                <Text>Interval: {integration.syncInterval} min</Text>
                              )}
                          ...
                          <VStack spacing={2}>
                            ...
                            {integration.autoSync && (
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => handleSyncIntegration(integration.name)}
                        ...
                        {status && (
                          <Box mt={4} p={3} bg={cardBg} borderRadius="lg">
                            ...
                            <HStack spacing={4} fontSize="xs" color="gray.500">
                              ...
                              {status.lastSync && (
                                <Text>Last sync: {status.lastSync.toLocaleString()}</Text>
                              )}
                              {status.errorCount > 0 && (
                    ...
                    <CardBody>
                      <VStack spacing={4} align="stretch">
                        <Stat>
                          ...
                          <StatNumber color="blue.600">{metrics.totalRequests}</StatNumber>
                        </Stat>
          ...
          <TabPanel>
            <Box textAlign="center" py={12}>
              ...
              <Text color="gray.500">
                Configure API keys, endpoints, and sync settings for each integration. Coming soon with full configuration management.
              </Text>
            </Box>
          </TabPanel>
          ...
          <TabPanel>
            <Box textAlign="center" py={12}>
              ...
              <Text color="gray.500">
                Comprehensive analytics and performance metrics for all API integrations. Coming soon with detailed charts and insights.
              </Text>
            </Box>
          </TabPanel>
          ...
          <TabPanel>
            <Box textAlign="center" py={12}>
              ...
              <Text color="gray.500">
                Detailed logs and error tracking for all API operations. Coming soon with comprehensive logging and debugging tools.
              </Text>
            </Box>
          </TabPanel>
        </TabPanels>
            ...
            {selectedIntegration && (
              <VStack spacing={4} align="stretch">
                ...
                <FormControl>
                  ...
                  />
                  <FormHelperText>Automatically sync data at regular intervals</FormHelperText>
                </FormControl>
                ...
                {selectedIntegration.autoSync && (
                  <FormControl>
                    ...
                    <FormHelperText>How often to sync data (1-1440 minutes)</FormHelperText>
                  </FormControl>
                )}
                ...
                <FormControl>
                  ...
                  <FormHelperText>Request timeout in milliseconds</FormHelperText>
                </FormControl>
              </VStack>
            )}
          </ModalBody>
          <ModalFooter>
            <Button variant="ghost" mr={3} onClick={() => setIsConfigModalOpen(false)}>
              Cancel
            </Button>
