...
export const MobileRouteEditor: React.FC<MobileRouteEditorProps> = ({
  ...
  const drawRoute = useCallback((ctx: CanvasRenderingContext2D, route: Route, isSelected: boolean, isEditing = false) => {
  ...
  const findPointAtPosition = useCallback((x: number, y: number, threshold = 20): RoutePoint | null => {
  ...
  const handlePointMove = useCallback((pointId: string, newX: number, newY: number) => {
  ...
  const handleRouteColorChange = useCallback((routeId: string, color: string) => {
    const updatedRoutes = routes.map(route =>
  ...
  const handleRouteTypeChange = useCallback((routeId: string, type: 'run' | 'pass' | 'block') => {
    const updatedRoutes = routes.map(route =>
      ...
      {showColorPicker && (
        ...
        >
          {colors.map(color => (
            <TouchableOpacity
              key={color}
              onPress={() => selectedRouteId && handleRouteColorChange(selectedRouteId, color)}
      ...
      {showRouteTypeSelector && (
        ...
        >
          {(['run', 'pass', 'block'] as const).map(type => (
            <TouchableOpacity
              key={type}
              onPress={() => selectedRouteId && handleRouteTypeChange(selectedRouteId, type)}
